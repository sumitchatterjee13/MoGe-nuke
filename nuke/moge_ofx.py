...
def default_model():
    for name in ("moge-3-vitg.pt", "moge-3-vitg.safetensors"):
...
def start(port=DEFAULT_PORT, model=None, python_exe=DEFAULT_PYTHON, exit_with_nuke=True):
    ...
    for label, path in (("python", python_exe), ("daemon", DAEMON)):
        if not os.path.isfile(path):
            nuke.message("{0} not found:\n{1}\n\nRun install.ps1 first.".format(label, path))
            return
    env = dict(os.environ)
    ...
    subprocess.Popen(cmd, env=env, cwd=os.path.dirname(DAEMON), creationflags=flags)
    print("[MoGe3] daemon starting: " + " ".join(cmd))
    nuke.message("MoGe daemon starting on port {0} (own console window).\n"
                 "The model takes 15-60 s to load.".format(port))
